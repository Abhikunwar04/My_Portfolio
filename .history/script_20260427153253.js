...
function canvasDraw() {
  ...
  const main = document.getElementById('mainContent'); // ✅ FIX: missing reference
  ...
  const zoom = 1.35 + (1.0 - 1.35) * easeOut(p);
  ...
  if (!entryDone) {
    ...
    textY = startY + (finalY - startY) * easeOut(entryProgress);
    nameAlpha = entryProgress;

    if (entryProgress >= 1) {
      entryProgress = 1;
      entryDone = true; // ✅ FIX: important
    }
  } else {
    const sp = clamp((p - 0.05) / 0.7, 0, 1);
    textY = finalY + (H * 0.35) * easeOut(sp);
    nameAlpha = clamp((p - 0.05) / 0.3, 0, 1);
  }

  /* ---------------- MASK CANVAS SAFE ---------------- */
  ensureMaskCanvas(W, H, dpr);

  if (!mc || !mctx) return; // ✅ FIX: prevent crash
  ...
  // reset composite mode (IMPORTANT)
...
function canvasTick(now) {
  ...
  // ✅ ENTRY ANIMATION (independent — scroll block nahi karega)
  if (!entryDone && !document.body.classList.contains('loading')) {
    entryProgress += 0.025;
  ...
  // ✅ SCROLL ALWAYS ACTIVE (IMPORTANT FIX)
