...
/* ── 3D FLOATING ELEMENTS ON CURSOR ── */
const sections = document.querySelectorAll('#about, .skills-section, #projects, #contact');

const shapeConfigs = {
  'about':          { color: '0, 200, 255',  shapes: ['cube', 'ring', 'triangle'] },
  'skills-section': { color: '0, 255, 180',  shapes: ['diamond', 'ring', 'dot']   },
  'projects':       { color: '160, 0, 255',  shapes: ['cube', 'diamond', 'ring']  },
  'feedback-section': { color: '255, 120, 0', shapes: ['triangle', 'dot', 'ring'] }
};

sections.forEach(section => {
  const id     = section.id || section.className.split(' ')[0];
  const config = shapeConfigs[id] || { color: '0,229,255', shapes: ['ring','dot','cube'] };
  const [r,g,b] = config.color.split(',').map(s => s.trim());

  // 6 shapes per section banao
  const shapesArr = [];
  for (let i = 0; i < 6; i++) {
    const el  = document.createElement('div');
    const type = config.shapes[i % config.shapes.length];
    el.className  = `float-shape shape-${type}`;
    el.style.cssText = `
      position: absolute;
      pointer-events: none;
      z-index: 0;
    ...
    shapesArr.push({ el, baseX: 10 + i * 15, baseY: 20 + (i % 3) * 25 });
  }
  ...
  section.addEventListener('mousemove', (e) => {
    ...
    shapesArr.forEach(({ el, baseX, baseY }, i) => {
      ...
      if (dist < range) {
        ...
        const rotate   = strength * 45 * (i % 2 === 0 ? 1 : -1);

        el.style.opacity   = Math.min(strength * 1.5, 0.85);
