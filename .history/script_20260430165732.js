...
if (!revealDone && rawEntry >= BOOM_POINT) {
  revealDone = true;
  boomStartTime = now;   // 💥 store exact frame time
  onNameSettled();
}
 ...
 // ── STEP 7: Boom shock rings (TIME BASED) ──
if (boomStartTime) {
  const elapsed = now - boomStartTime;

  // fast + punchy timing (OLD FEEL)
  const shockT = clamp(elapsed / 400, 0, 1); // ⚡ faster than before

  const centerX = W / 2;
  const centerY = textY + nameH / 2;

  // 💥 HARD IMPACT RINGS (old jaisa feel)
  [0, 0.08, 0.16].forEach((delay, i) => {
    const t = clamp((shockT - delay) / (1 - delay), 0, 1);
    if (t <= 0) return;

    // ⚡ less smooth easing (more punch)
    const eased = t * (2 - t); // sharper than easeOut

    const radius = eased * W * (0.6 + i * 0.25);
    ...
    ctx.shadowBlur  = 35; // 🔥 more glow = more impact
    ctx.stroke();
    ctx.restore();
  });

  // ⚡ SHARP FLASH (old wali vibe)
  if (shockT < 0.15) {
    const flash = 1 - shockT / 0.15;

    ctx.save();
    ctx.fillStyle = `rgba(0, 229, 255, ${flash * 0.5})`; // stronger
    ctx.fillRect(0, 0, W, H);
    ctx.restore();
  }

  // 💥 HARD SHAKE (important for "maza")
  if (shockT < 0.25) {
    const shake = (1 - shockT / 0.25) * 12; // ⚡ stronger shake
