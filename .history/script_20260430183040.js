...
// ── STEP 7: Smooth Cinematic Boom Effect ──
if (revealDone && boomStartTime && !nameEntryDone) {

  const elapsed = performance.now() - boomStartTime;
  const DURATION = 2200;
  ...
  // 1. HARD FLASH — sharp white hit, 160ms mein khatam
  if (elapsed < 160) {
    const ft = elapsed / 160;
    const fa = Math.pow(1 - ft, 2.5) * 0.72;
    ctx.save();
    ctx.fillStyle = `rgba(220,248,255,${fa})`;
    ctx.fillRect(0, 0, W, H);
    ctx.restore();
  }

  // 2. RADIAL BLOOM — soft linger
  if (T < 0.55) {
    const bt   = T / 0.55;
    const ba   = Math.pow(1 - bt, 2.8) * 0.22;
    const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, W * 0.85);
    grad.addColorStop(0,    `rgba(0,229,255,${ba * 3})`);
    grad.addColorStop(0.25, `rgba(0,180,255,${ba * 1.5})`);
    grad.addColorStop(0.7,  `rgba(0,80,120,${ba * 0.4})`);
  ...
  // 3. SHOCKWAVE RINGS — triple pass (glow + mid + crisp core)
  const rings = [
    [0.00, 0.50, 0.75, 1.00, 2.2, [0,229,255]],
    [0.03, 0.52, 0.62, 0.70, 1.4, [255,255,255]],
    [0.07, 0.55, 0.80, 0.85, 1.8, [0,210,255]],
    [0.12, 0.50, 0.50, 0.55, 1.0, [160,240,255]],
    [0.18, 0.58, 0.88, 0.72, 1.6, [0,229,255]],
    [0.26, 0.52, 0.60, 0.45, 0.9, [255,255,255]],
    [0.36, 0.50, 0.70, 0.38, 1.2, [0,200,255]],
  ];

  rings.forEach(([st, dur, rFactor, peakA, lw, col]) => {
    const localT = clamp((T - st) / dur, 0, 1);
    if (localT <= 0) return;

    const eased  = easeOutExpo(localT);
    const radius = eased * W * rFactor * 0.5;

    const riseT = clamp(localT / 0.12, 0, 1);
    const fallT = clamp((localT - 0.12) / 0.88, 0, 1);
    const a     = peakA * Math.min(riseT, Math.pow(1 - fallT, 1.6));
    if (a < 0.005) return;
  ...
  // 4. PARTICLES — golden-ratio spread
  if (T < 0.80) {
    for (let i = 0; i < 28; i++) {
      const angle  = i * 2.39996;
      const speed  = 0.12 + (i * 137 % 100) / 100 * 0.28;
      const life   = 0.28 + (i * 71  % 100) / 100 * 0.52;
      const lt     = clamp(T / life, 0, 1);
      if (lt >= 1) continue;

      const dist  = easeOutCubic(lt) * W * speed * 0.5;
      const px    = centerX + Math.cos(angle) * dist;
      const py    = centerY + Math.sin(angle) * dist;
      const pa    = Math.pow(1 - lt, 2.2) * 0.9;
      const psize = (1 - lt) * (1.8 + (i % 3) * 0.8);
      if (pa < 0.01) continue;
  ...
  // 5. LIGHT STREAKS — tapered gradient
  if (T < 0.45) {
    for (let si = 0; si < 5; si++) {
      const st_delay = si * 0.04;
      const lt       = clamp((T - st_delay) / (0.45 - st_delay), 0, 1);
      if (lt <= 0) continue;

      const sy    = centerY + (si - 2) * 26;
      const fadeA = Math.pow(1 - lt, 2.8) * (0.40 - si * 0.05);
      if (fadeA < 0.005) continue;

      const halfW = easeOutCubic(lt) * W * (0.38 + si * 0.03) * (1 - lt * 0.35);
      const grad  = ctx.createLinearGradient(centerX - halfW, sy, centerX + halfW, sy);
      grad.addColorStop(0,    `rgba(0,229,255,0)`);
      grad.addColorStop(0.25, `rgba(0,229,255,${fadeA})`);
      grad.addColorStop(0.50, `rgba(210,248,255,${fadeA * 1.6})`);
      grad.addColorStop(0.75, `rgba(0,229,255,${fadeA})`);
  ...
  if (T > 0.03 && T < 0.45) {
    const ct    = clamp((T - 0.03) / 0.42, 0, 1);
    const baseR = easeOutExpo(ct) * W * 0.36;
    const ca    = Math.pow(1 - ct, 2.8) * 0.28;
    const off   = 6 + ct * 18;
    if (ca > 0.005) {
      ctx.save();
      ctx.beginPath();
      ctx.arc(centerX - off, centerY, baseR, 0, Math.PI * 2);
      ctx.strokeStyle = `rgba(255,60,80,${ca})`;
      ctx.lineWidth   = 1.0;
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(centerX + off, centerY, baseR, 0, Math.PI * 2);
      ctx.strokeStyle = `rgba(0,255,210,${ca})`;
      ctx.lineWidth   = 1.0;
      ctx.stroke();
      ctx.restore();
    }
