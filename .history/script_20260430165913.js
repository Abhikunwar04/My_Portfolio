...
if (!revealDone && rawEntry >= BOOM_POINT) {
 ...
 // ── BOOM EFFECT (FIXED - SAME STYLE) ──
if (boomStartTime) {
  const elapsed = now - boomStartTime;

  // ⚡ same duration feel, but stable
  const shockT = clamp(elapsed / 500, 0, 1);

  const centerX = W / 2;
  const centerY = textY + nameH / 2;

  // 💥 SHOCK RINGS (same pattern, smoother timing)
  [0, 0.1, 0.2, 0.32, 0.45].forEach((delay, i) => {
    ...
    const radius = eased * W * (0.55 + i * 0.15);
  ...
  // ⚡ FLASH (same sine feel, but controlled)
  if (shockT < 0.25) {
  ...
  // ⚡ ENERGY LINES (same but smoother spread)
  if (shockT < 0.4) {
    const eased = easeOut(shockT);
    const lineAlpha = (1 - shockT / 0.4) * 0.4;

    for (let li = 0; li < 8; li++) {
      const lineY = centerY + (li - 4) * (nameH * 0.3);
      const lineW = eased * W * (0.4 + li * 0.05);
  ...
  // 💥 CAMERA SHAKE (added = MORE MAZA 😈)
  if (shockT < 0.3) {
    const shake = (1 - shockT / 0.3) * 10;
