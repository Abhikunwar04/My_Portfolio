...
window.addEventListener('load', () => {
  ...
  drops.length = 0;
  for (let i = 0; i < newCols; i++) drops.push(Math.random() * -50);
}
  }
  preloaderResize();
...
const COL_SIZE = 16;
const cols = Math.floor(PW / COL_SIZE);
const drops = Array.from({ length: cols }, () => Math.random() * -50);
  ...
  function drawMatrix() {
    ...
    drops.forEach((y, i) => {
      ...
      c2.fillStyle = Math.random() > 0.88 ? 'rgba(220,255,220,1.0)' : 'rgba(0,255,65,0.92)';
      c2.fillText(text, i * COL_SIZE, y * COL_SIZE);
