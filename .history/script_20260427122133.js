...
function updateHeroText(p) {
 Text) return;
  ...
  if (p > 0.72) {
  const fadeIn  = Math.min((p - 0.72) / 0.12, 1);
  const fadeOut = p > 0.90 ? Math.max(1 - (p - 0.90) / 0.08, 0) : 1;
  heroText.style.opacity = fadeIn * fadeOut;
  ...
  if (resume) {
    ...
    if (rect.top <  const heroText = document.getElementById('hero');
...
if (main) {
if (main && !mainSettled) {
  // p = 0 → mainContent screen ke bilkul niche (100vh)
  // p = 1 → mainContent upar aa jaaye (0)
  const slideProgress = Math.max(p, 0);
  const translateY = (1 - Math.min(slideProgress, 1)) * 100;
  main.style.transform = `translateY(${translateY}vh)`;
  ...
  vBot.addColorStop(1.0, 'rgba(0,0,0,0.72)');
