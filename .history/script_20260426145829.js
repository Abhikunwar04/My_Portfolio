...
function updateMainContent(p) {
  ...
  const ease       = easeOut(clamp((p - 0.30) / 0.70, 0, 1));
  const translateY = (1 - ease) * window.innerHeight;
  ...
  if (p >= 0.999) {
    mainSettled = true; heroTextSettled = true;
    main.classList.add('settled');
    main.style.transform = '';
    main.style.opacity   = '';
    if (heroText) { heroText.classList.add('settled'); heroText.style.opacity = ''; }
    if (hCanvas)  { hCanvas.style.display = 'none'; }
    main.querySelectorAll('*').forEach(el => { el.style.visibility = 'visible'; });
  }
}
...
function canvasDraw() {
  ...
  const zoom    = 1.65 + (1.00 - 1.65) * easeOut(p);
  const scaledW = imgW * zoom;
  const scaledH = imgH * zoom;
  const baseX   = (W - scaledW) * 0.8;
...
function canvasTick(now) {
  ...
  if (smoothP < 0.97 && mainSettled) {
    mainSettled = false; heroTextSettled = false;
    const main     = document.getElementById('mainContent');
    const heroText = document.getElementById('hero');
    const hCanvas  = document.getElementById('heroCanvas');
