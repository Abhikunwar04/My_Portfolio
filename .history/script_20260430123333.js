...
const NAME_ENTRY_DURATION = 1200; 
...
const entryT = easeOut(nameEntryProgress);
const scrollT = easeOut(clamp((p - 0.05) / 0.7, 0, 1));

const entryStartY = -nameH; // upar se bahar se aaye
const entryEndY   = H * 0.50 - nameH / 2; // middle mein settle

const scrollTextY = (H * 0.85 - nameH / 2)
                  + (entryEndY - (H * 0.85 - nameH / 2))
                  * scrollT;

const textY = nameEntryDone
  ? scrollTextY
  : entryStartY + (entryEndY - entryStartY) * entryT;

const nameAlpha = nameEntryDone
  ? clamp((p - 0.05) / 0.3, 0, 1)
  : entryT;
      ...
      runProgress(900, () => {
        setTimeout(() => {
          ...
          setTimeout(() => {
           ...
           canvasInit(); /* ← Preloader khatam — ab heroCanvas shuru karo */
          }, 700);
        }, 300);
      });
    }, 400);
