...
function preloaderResize() {
  ...
  for (let i = 0; i < newCols; i++) 
   drops.push({ y: -(8 + Math.floor(Math.random() * 12)) - Math.random() * 50, len: 8 + Math.floor(Math.random() * 12) });
}
preloaderResize();
...
drawMatrix();
  ...
  drops.forEach((drop, i) => {
    for (let j = 0; j < drop.len; j++) {
      const yPos = (drop.y - j) * COL_SIZE;
      if (yPos < 0 || yPos > PH) continue;
      
      const alpha = 1 - (j / drop.len);  // head bright, tail fade
      const text = CHARS[Math.floor(Math.random() * CHARS.length)];
      
      if (j === 0) {
        c2.fillStyle = `rgba(220,255,220,1.0)`;  // head — white
      } else {
        c2.fillStyle = `rgba(0,255,65,${alpha * 0.9})`;  // trail — green fade
      }
      
      c2.font = "14px 'Share Tech Mono', monospace";
      c2.fillText(text, i * COL_SIZE, yPos);
    }
    
    drop.y += 0.3;
    if (drop.y * COL_SIZE > PH + drop.len * COL_SIZE) {
  drop.y = -(drop.len + Math.random() * 10);
  drop.len = 8 + Math.floor(Math.random() * 12);
}
  });
