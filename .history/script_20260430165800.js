...
if (boomStartTime) {
  ...
  let shockT = clamp(elapsed / 600, 0, 1); // 600ms duration

  const centerX = W / 2;
  const centerY = textY + nameH / 2;

  // 🔵 Shock Rings
  [0, 0.12, 0.25, 0.4].forEach((delay, i) => {
    const t = clamp((shockT - delay) / (1 - delay), 0, 1);
    if (t <= 0) return;

    const eased = easeOut(t);

    const radius = eased * W * (0.5 + i * 0.2);
    const alpha  = (1 - eased) * (0.6 - i * 0.1);
  ...
  // 💥 Cinematic Flash (NO sine flicker)
  if (shockT < 0.2) {
    const flash = 1 - shockT / 0.2;
    ctx.save();
    ctx.fillStyle = `rgba(0, 229, 255, ${flash * 0.25})`;
