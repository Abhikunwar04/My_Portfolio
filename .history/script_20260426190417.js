...
function updateActiveNav() {
  ...
  sections.forEach(id => {
    ...
    if (!el) return;
    if (window.scrollY + navH + 100 >= el.offsetTop) current = id;
  });
...
/* ============================================================
   3.  NAVBAR CLICK — BUG-FREE SCROLL
       Fix: wait for mainContent layout to fully settle before calculating offsets
   ============================================================ */
document.querySelectorAll('a[href^="#"]').forEach(link => {
  link.addEventListener('click', function(e) {
    ...
    const doSnapScroll = () => {
      /* mainContent is the snap container — scroll inside it */
      const mc = document.getElementById('mainContent');
      if (!mc) return;
      /* target offset relative to mainContent */
    ...
    if (window.scrollY < heroScrollEnd - window.innerHeight * 0.5) {
      /* Jump past hero zone first */
      window.scrollTo({ top: heroScrollEnd, behavior: 'instant' });
      /* Wait 3 frames for layout recalc then snap-scroll inside mainContent */
      requestAnimationFrame(() => requestAnimationFrame(() => requestAnimationFrame(doSnapScroll)));
    } else {
      doSnapScroll();
    }
  });
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof AOS !== 'undefined') {
    AOS.init({
      ...
      once: true,
      offset: 80,
      /* use mainContent as scroll container for AOS */
      container: document.getElementById('mainContent') || window
    });
  }
...
(function initCinematicBg() {
  const sectionCfg = [
    {
      ...
      base: ['#04030c', '#060510', '#050408'],
      accents: [
        { x: 0.50, y: 0.50, r: 700, c: 'rgba(0,229,255,0.06)' },
        { x: 0.80, y: 0.20, r: 300, c: 'rgba(60,0,120,0.18)' }
      ]
    },
    {
      selector: '.about-section',
      base: ['#05040e', '#070618', '#060709'],
      accents: [
        { x: 0.85, y: 0.15, r: 650, c: 'rgba(40,0,110,0.35)' },
        { x: 0.05, y: 0.75, r: 500, c: 'rgba(0,40,120,0.22)' },
        { x: 0.50, y: 1.00, r: 400, c: 'rgba(0,229,255,0.05)' }
      ]
    },
    {
      selector: '.skills-section',
      base: ['#040610', '#03040c', '#040610'],
      accents: [
        { x: 0.50, y: 0.50, r: 720, c: 'rgba(0,60,140,0.20)' },
        { x: 0.02, y: 0.10, r: 350, c: 'rgba(0,110,80,0.13)' },
        { x: 0.98, y: 0.90, r: 300, c: 'rgba(80,0,120,0.12)' }
      ]
    },
    {
      selector: '#projects',
      base: ['#040807', '#050708', '#06050c'],
      accents: [
        { x: 0.90, y: 0.30, r: 600, c: 'rgba(0,80,60,0.20)' },
        { x: 0.08, y: 0.75, r: 420, c: 'rgba(65,0,110,0.18)' },
        { x: 0.50, y: 0.00, r: 350, c: 'rgba(0,229,255,0.04)' }
      ]
    },
    {
      selector: '.feedback-section',
      base: ['#040610', '#050608', '#050410'],
      accents: [
        { x: 0.50, y: 0.20, r: 800, c: 'rgba(0,30,90,0.28)' },
        { x: 0.88, y: 0.88, r: 380, c: 'rgba(90,0,70,0.18)' },
        { x: 0.10, y: 0.50, r: 320, c: 'rgba(0,229,255,0.04)' }
      ]
    }
  ];
  ...
  sectionCfg.forEach(cfg => {
    ...
    function draw() {
      ...
      const imageData = gc.createImageData(200, 200);
      ...
      const pattern = c.createPattern(grainCanvas, 'repeat');
