...
function canvasDraw() {
  ...
  const bottomShiftY = p * scaledH * 0.18;
...
if (main && !mainSettled) {
  // p=0 → niche (100vh), p=0.5 → aadha upar, p=1 → bilkul upar
  const slide = clamp(p / 0.85, 0, 1);
  const eased = easeOut(slide);
  const ty = (1 - eased) * 100;
  main.style.transform = `translateY(${ty}vh)`;
}
  /* Vignettes */
...
if (smoothP > 0.98 && !mainSettled) {
  mainSettled = true;
  const main = document.getElementById('mainContent');
  if (main) {
    main.style.transform = 'translateY(0)';
    setTimeout(() => main.classList.add('settled'), 100);

    // thodi der baad settled class dalo taaki transition smooth ho
    setTimeout(() => {
...
// hero text ab mainContent mein hai — seedha start karo
initTypingEffect();
});
