...
function updateHeroText(p) {
  ...
  /*
    Logic:
    - jab resume bottom se enter kare → fade start
    - jab center tak aa jaye → fully hidden
  */

  const startFade = vh * 0.9;  // fade start (screen ke neeche se)
  const endFade   = vh * 0.4;  // fade end (screen ke beech tak)

  const y = rect.top;

  if (y < startFade && y > endFade) {
    const progress = (startFade - y) / (startFade - endFade);
