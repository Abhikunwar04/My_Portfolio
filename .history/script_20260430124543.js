...
// Bounce overshoot — settle hone pe thoda bounce
const bounceT = rawEntry < 0.7
  ? easeOut(rawEntry / 0.7)
  : 1 + Math.sin((rawEntry - 0.7) / 0.3 * Math.PI) * 0.04 * (1 - rawEntry);
...
// Glitch blink effect — entry ke beech mein flicker
const flickerZones = [0.3, 0.5, 0.65];
const isFlicker = !nameEntryDone && flickerZones.some(z =>
  Math.abs(rawEntry - z) < 0.03 && Math.random() > 0.4
);

// Wave scale — settle hone pe pulse
const settleProgress = clamp((rawEntry - 0.75) / 0.25, 0, 1);
const waveScale = nameEntryDone
  ? 1
  : rawEntry > 0.75
    ? 1 + Math.sin(settleProgress * Math.PI) * 0.18  // boom scale up
    : 1;

const nameAlpha = nameEntryDone
  ? clamp(1 - (p - 0.5) / 0.4, 0, 1)
  : isFlicker ? Math.random() * 0.5 : entryT;
...
mctx.restore();
  ...
  // SHOCKWAVE EFFECT — jab name settle ho
if (!nameEntryDone && rawEntry > 0.75) {
  const shockT = clamp((rawEntry - 0.75) / 0.25, 0, 1);
  const shockEase = easeOut(shockT);
  const centerX = W / 2;
  const centerY = textY + nameH / 2;
  
  // 3 rings — ek ke baad ek
  [0, 0.15, 0.3].forEach((delay, i) => {
    const t = clamp((shockT - delay) / (1 - delay), 0, 1);
    if (t <= 0) return;
    const radius = easeOut(t) * W * 0.6;
    const alpha  = (1 - t) * (0.5 - i * 0.12);
    if (alpha <= 0) return;
