...
window.addEventListener('scroll', () => {
  ...
  if (currentScroll > 10) topBar.classList.add('scrolled');
...
========================= */
function createRevealObserver(el, className) {
  if (!el) return;
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        el.classList.remove(className); // 🔥 THIS IS KEY (repeat animation)
      }
    });
  }, { threshold: 0.4 });
...
createRevealObserver(document.getElementById('hero'), 'reveal');
createRevealObserver(document.querySelector('.social-icons'), 'reveal');
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof AOS !== "undefined") {
    AOS.init({ duration: 800, once: false }); // 🔥 repeat enabled
  }
...
/* =========================
   GSAP ABOUT (RESTORED PREMIUM + REPEAT SAFE)
========================= */
