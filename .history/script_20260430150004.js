...
function canvasDraw() {
  ...
  // ── NAME ENTRY PROGRESS UPDATE (pehle calculate karo) ──
  if (!nameEntryDone) {
    if (!nameEntryReady) {
      nameEntryProgress = 0;
    } else {
  ...
  // ── BOOM TRIGGER — ek baar ──
  if (!nameEntryDone && rawEntry > 0.72 && !window._revealDone) {
  ...
  if (nameEntryDone) {
    // Normal: full image with scroll parallax
    ctx.drawImage(hero, baseX, baseY - heroShiftY, scaledW, scaledH);

  } else if (rawEntry > 0.72) {
    // Boom ke saath: center se scale out karte hue reveal
  ...
  // rawEntry <= 0.72: sirf black — kuch draw nahi
  ...
  const bottomShiftY = nameEntryDone ? p * scaledH * 0.12 : 0;
  ...
  if (nameEntryDone) {
    ...
    // Boom ke saath bottom gradually reveal
