...
if (nameEntryDone) {
  // Normal scroll ke baad — full image
  ctx.drawImage(hero, baseX, baseY - heroShiftY, scaledW, scaledH);

} else if (rawEntry > 0.72) {
  // ✅ Boom ke saath hero reveal — center se scale out
  const revealT  = clamp((rawEntry - 0.72) / 0.28, 0, 1);
  const revealE  = easeOut(revealT);          // fast expand

  // Hero boom center = name ka center (screen middle)
  const cx = W / 2;
  const cy = H / 2;

  // Scale: 0 se start, 1 pe full size
  const heroScale = 0.3 + revealE * 0.7;     // 0.3 → 1.0
  const heroAlpha = revealE;                  // 0 → 1

  const drawW = scaledW * heroScale;
  const drawH = scaledH * heroScale;
  const drawX = cx - drawW / 2 + (baseX - W/2 + scaledW/2) * revealE;
  const drawY = cy - drawH / 2 + (baseY - H/2 + scaledH/2) * revealE;
 ...
 // Entry animation — upar se aata hai
if (!nameEntryDone) {
  if (!nameEntryReady) {
    nameEntryProgress = 0;
  } else {
    if (!nameEntryStart) nameEntryStart = performance.now();
    nameEntryProgress = Math.min((performance.now() - nameEntryStart) / NAME_ENTRY_DURATION, 1);
    if (nameEntryProgress >= 1) {
  nameEntryDone = true;
}
  }
}
...
const entryT   = easeOut(rawEntry);

// Smooth entry — blink bilkul nahi
...
ensureMaskCanvas(W, H, dpr);
  ...
  // ✅ CHANGE 1: bottom mask SIRF tab draw karo jab name settle ho chuka ho
  if (nameEntryDone) {
  ...
  // ✅ Boom ke saath bottom bhi reveal — same revealT se
  const revealT = clamp((rawEntry - 0.72) / 0.28, 0, 1);
  const revealE = easeOut(revealT);
  const bottomShiftY = 0; // scroll nahi hua abhi

  // Bottom mask same animation ke saath
  mctx.globalCompositeOperation = 'destination-out';
  mctx.globalAlpha = revealE;              // ✅ gradually cut karo
  mctx.drawImage(bottom, baseX, baseY - bottomShiftY, scaledW, scaledH);
  mctx.restore();
  ctx.drawImage(mc, 0, 0, mc.width, mc.height, 0, 0, W, H);
  ctx.save();
  ...
  // Name animation only — kuch reveal nahi
  mctx.restore();
  ctx.drawImage(mc, 0, 0, mc.width, mc.height, 0, 0, W, H);
}

if (!nameEntryDone && rawEntry > 0.72) {
   if (!window._revealDone) {
    window._revealDone = true;
    onNameSettled();
  }
  const shockT  = clamp((rawEntry - 0.72) / 0.28, 0, 1);
  const centerX = W / 2;
      ...
      runProgress(900, () => {
        setTimeout(() => {
          ...
          setTimeout(() => {
           ...
           document.body.scrollTop = 0;
           nameEntryReady = true;
           nameEntryStart = performance.now();
           canvasInit();
          }, 700);
        }, 300);
      });
