...
function updateMainContent(p) {
  ...
  const slideProgress = clamp((p - 0.40) / 0.60, 0, 1);
  const ease = easeOut(slideProgress);

const vh = window.innerHeight;
const translateY = (1 - ease) * vh;
const opacity = clamp((p - 0.40) / 0.35, 0, 1);
 ...
 if (p >= 0.999) {
    ...
    // Sab inner elements force visible
...
document.querySelectorAll('.nav-links a').forEach(link => {
  link.addEventListener('click', (e) => {
    ...
    if (isHome) {
      // Home click — hero animation ke saath top pe
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }

    if (inHeroZone) {
      // Hero zone mein hain — pehle hero scroll complete karo, phir jump
      window.scrollTo({ top: heroEnd + 10, behavior: 'smooth' });
      setTimeout(() => {
        if (target) target.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 1000);
    } else {
      // Already hero ke baad — seedha jump
      if (target) target.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  });
});
