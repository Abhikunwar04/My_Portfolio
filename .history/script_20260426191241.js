...
/* Tell GSAP ScrollTrigger about our custom snap-scroll container */
window.addEventListener('DOMContentLoaded', () => {
  const mc = document.getElementById('mainContent');
  if (mc) {
    ScrollTrigger.scrollerProxy('#mainContent', {
      scrollTop(value) {
        if (arguments.length) { mc.scrollTop = value; }
        return mc.scrollTop;
      },
      getBoundingClientRect() {
        return { top: 0, left: 0, width: window.innerWidth, height: window.innerHeight };
      },
      pinType: mc.style.transform ? 'transform' : 'fixed'
    });
    mc.addEventListener('scroll', ScrollTrigger.update, { passive: true });
    ScrollTrigger.defaults({ scroller: '#mainContent' });
  }
});
...
function canvasTick(now) {
  ...
  if (smoothP > 0.98 && !heroTextSettled) {
    ...
    if (main) {
      /* Class le lega control — inline styles hatao */
...
function handleNavbarScroll() {
  ...
  /* Scrolled style — show when either window scrolled OR mainContent scrolled */
  const mcScrolled = mc ? mc.scrollTop > 20 : false;
  topBar.classList.toggle('scrolled', cur > 20 || mcScrolled);

  /* Hide on scroll down (only for window scroll — hero zone) */
...
/* Also listen to mainContent scroll for active link + navbar style */
window.addEventListener('DOMContentLoaded', () => {
  const mc = document.getElementById('mainContent');
  if (mc) {
...
function updateActiveNav() {
  ...
  sections.forEach(id => {
    ...
    if (!el) return;
    /* getBoundingClientRect works for both window and snap-container children */
...
/* ============================================================
   3.  NAVBAR CLICK — SNAP SCROLL FIX
       mainContent ek snap scroll container hai (height:100vh, overflow-y:scroll)
       isliye window.scrollTo kaam nahi karta — mainContent.scrollTo chahiye
   ============================================================ */
document.querySelectorAll('a[href^="#"]').forEach(link => {
  link.addEventListener('click', function(e) {
    ...
    function doSnapScroll() {
      if (!mc) return;
      const mcRect   = mc.getBoundingClientRect();
      const tRect    = target.getBoundingClientRect();
      const scrollTo = mc.scrollTop + (tRect.top - mcRect.top);
      mc.scrollTo({ top: Math.max(0, scrollTo), behavior: 'smooth' });
    }

    if (window.scrollY < heroScrollEnd - window.innerHeight * 0.4) {
      /* Hero zone se instantly bahar, phir mainContent ke andar snap */
      window.scrollTo({ top: heroScrollEnd, behavior: 'instant' });
      requestAnimationFrame(() =>
        requestAnimationFrame(() =>
          requestAnimationFrame(doSnapScroll)
        )
      );
    } else {
      doSnapScroll();
...
/* ============================================================
   8.  GSAP SCROLL ANIMATIONS
       scroller = #mainContent (snap container ke andar hai sab)
   ============================================================ */
const MC_SCROLLER = '#mainContent';

gsap.from('.about-text', {
  scrollTrigger: { trigger: '.about-section', scroller: MC_SCROLLER, start: 'top 82%', once: true },
...
gsap.from('.project-card', {
  scrollTrigger: { trigger: '#projects', scroller: MC_SCROLLER, start: 'top 82%', once: true },
  opacity: 0, y: 60, stagger: 0.15, duration: 0.8, ease: 'power3.out'
});
gsap.from('.contact-form-wrapper', {
  scrollTrigger: { trigger: '#contact', scroller: MC_SCROLLER, start: 'top 82%', once: true },
  opacity: 0, y: 40, duration: 0.9, ease: 'power3.out'
});
gsap.from('.resume-wrapper', {
  scrollTrigger: { trigger: '.resume-wrapper', scroller: MC_SCROLLER, start: 'top 85%', once: true },
