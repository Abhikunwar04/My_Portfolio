...
function updateHeroText(p) {
  ...
  const heroLeftLottie = document.querySelector('.hero-lottie-left');
  if (!heroText) return;

  if (p > 0.7) {
    heroText.style.opacity = 1;
    heroCanvas.style.filter = 'blur(6px) brightness(0.5)';  // ← ADD
    if (heroLeftLottie) heroLeftLottie.style.opacity = 1;
    if (heroRightLottie) heroRightLottie.style.opacity = 1;
  } else {
    heroText.style.opacity = 0;
    heroCanvas.style.filter = 'none';  // ← ADD
    if (heroLeftLottie) heroLeftLottie.style.opacity = 0;
