...
if (!revealDone && prevRawEntry < BOOM_POINT && rawEntry >= BOOM_POINT) {
  ...
  // ── STEP 7: Boom shock (FIXED & STABLE) ──
if (boomStartTime) {
  const elapsed = now - boomStartTime;
  const duration = 900; // total boom duration (ms)
  const shockT = clamp(elapsed / duration, 0, 1);

  const centerX = W / 2;
  const centerY = textY + nameH / 2;

  // Rings
  [0, 0.08, 0.16, 0.28, 0.4].forEach((delay, i) => {
    const t = clamp((shockT - delay) / (1 - delay), 0, 1);
    if (t <= 0) return;

    const radius = easeOut(t) * W * (0.5 + i * 0.12);
    const alpha  = (1 - t) * (0.8 - i * 0.12);
  ...
  // Flash
  if (shockT < 0.2) {
    const flashAlpha = Math.sin(shockT / 0.2 * Math.PI) * 0.4;
    ctx.save();
  ...
  // Lines
  if (shockT < 0.35) {
    const lineAlpha = (1 - shockT / 0.35) * 0.5;

    for (let li = 0; li < 8; li++) {
      const lineY = centerY + (li - 4) * (nameH * 0.25);
      const lineW = easeOut(shockT) * W * (0.35 + li * 0.05);
