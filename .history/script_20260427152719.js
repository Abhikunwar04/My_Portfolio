...
// 🎯 SAME FINAL POSITION (VERY IMPORTANT)
const finalY = H * 0.50 - nameH / 2;
...
if (!entryDone) {
  ...
  // 🔥 SCROLL (center se move kare — no jump)
  textY = finalY + (H * 0.85 - finalY)
...
function canvasTick(now) {
  // 🔥 ENTRY ANIMATION (runs only once on load)
// 🚫 preloader ke time entry run nahi hogi
if (!entryDone && !document.body.classList.contains('loading')) {
  entryProgress += 0.015;
}
  const dt        = lastTime ? Math.min(now - lastTime, 2 * 16.67) : 16.67;
  lastTime        = now;
 rawP = (entryDone && !document.body.classList.contains('loading'))
  ? getScrollProgress()
  : 0;
