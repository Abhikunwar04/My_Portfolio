...
if (!nameEntryStart) {
  nameEntryProgress = 0;
  return; // start set nahi hua — draw skip karo
}
...
// ── STEP 7: Professional Cinematic Boom Effect ──
if (revealDone && boomStartTime && !nameEntryDone) {

  const now_ms   = performance.now();
  const DURATION = 1400; // ms — longer = more cinematic
  const shockT   = clamp((now_ms - boomStartTime) / DURATION, 0, 1);

  const centerX = W / 2;
  const centerY = textY + nameH / 2;

  // ── 1. SOFT RADIAL BLOOM (instant, then slow fade) ──
  // Full-screen warm bloom — like a shockwave hitting atmosphere
  if (shockT < 0.6) {
    const bloomT = shockT / 0.6;
    const bloomAlpha = Math.pow(1 - bloomT, 2.2) * 0.18;
    const grad = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, W * 0.9);
    grad.addColorStop(0,   `rgba(0, 229, 255, ${bloomAlpha * 2.5})`);
    grad.addColorStop(0.3, `rgba(0, 180, 255, ${bloomAlpha})`);
  ...
  // ── 2. SCREEN FLASH (sharp white hit, instant decay) ──
  if (shockT < 0.18) {
    // Curve: sharp spike at 0, gone by 0.18
    const flashCurve = Math.pow(1 - shockT / 0.18, 3);
    const flashAlpha = flashCurve * 0.55;
    ctx.save();
  ...
  // ── 3. CONCENTRIC SHOCKWAVE RINGS (staggered, eased) ──
  // Each ring: different start time, travels outward, fades elegantly
  const RINGS = [
    { delay: 0.00, speedMul: 1.00, maxR: 0.70, lw: 2.5, color: [0, 229, 255],   alpha: 0.90 },
    { delay: 0.06, speedMul: 0.88, maxR: 0.55, lw: 1.5, color: [255, 255, 255], alpha: 0.50 },
    { delay: 0.13, speedMul: 0.95, maxR: 0.65, lw: 2.0, color: [0, 200, 255],   alpha: 0.75 },
    { delay: 0.22, speedMul: 0.80, maxR: 0.48, lw: 1.0, color: [180, 240, 255], alpha: 0.40 },
    { delay: 0.34, speedMul: 1.10, maxR: 0.80, lw: 1.8, color: [0, 229, 255],   alpha: 0.60 },
    { delay: 0.50, speedMul: 0.75, maxR: 0.42, lw: 0.8, color: [255, 255, 255], alpha: 0.30 },
  ];

  RINGS.forEach(ring => {
    const localT = clamp((shockT - ring.delay) / (1 - ring.delay + 0.001), 0, 1);
    if (localT <= 0) return;

    // easeOutQuart — fast expansion, elegant slow-down
    const eased  = 1 - Math.pow(1 - localT, 4);
    const radius = eased * W * ring.maxR;

    // Alpha: full at birth, smooth quadratic fadeout
    const fadeAlpha = Math.pow(1 - localT, 1.8) * ring.alpha;
    ...
    ctx.strokeStyle = `rgba(${r}, ${g}, ${b}, ${fadeAlpha * 0.25})`;
    ctx.lineWidth   = ring.lw * 6;
    ctx.stroke();

    // Core ring pass (crisp)
    ctx.beginPath();
    ctx.arc(centerX, centerY, radius, 0, Math.PI * 2);
  ...
  // ── 4. ENERGY PARTICLES (radial burst from center) ──
  // Small dots flying outward — gives "matter" to the explosion
  if (shockT < 0.75) {
    const PARTICLE_COUNT = 24;
    const seed = 42; // deterministic — same particles every frame
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      // Pseudo-random angle & speed using golden ratio spread
      const angle    = (i / PARTICLE_COUNT) * Math.PI * 2 + i * 0.618;
      const speed    = 0.15 + ((i * 137 + 29) % 100) / 100 * 0.25; // 0.15–0.40 of W
      const lifetime = 0.3 + ((i * 71 + 13) % 100) / 100 * 0.45;  // each particle lives 0.30–0.75 of shockT

      const localT = clamp(shockT / lifetime, 0, 1);
      if (localT <= 0 || localT >= 1) continue;

      // easeOutCubic travel
      const dist   = (1 - Math.pow(1 - localT, 3)) * W * speed;
      const px     = centerX + Math.cos(angle) * dist;
      const py     = centerY + Math.sin(angle) * dist;
      const pAlpha = Math.pow(1 - localT, 2) * 0.85;
      const pSize  = (1 - localT) * (2.5 + (i % 3));
      ...
      // Alternate between cyan and white particles
      ctx.fillStyle = i % 4 === 0
        ? `rgba(255, 255, 255, ${pAlpha * 0.7})`
        : `rgba(0, 229, 255, ${pAlpha})`;
  ...
  // ── 5. HORIZONTAL LIGHT STREAKS (cinematic lens flare feel) ──
  // These are NOT the crude lines from before — they taper at both ends
  if (shockT < 0.50) {
    const streakT = shockT / 0.50;
    const STREAKS = 5;

    for (let si = 0; si < STREAKS; si++) {
      const offset     = (si - (STREAKS - 1) / 2) * (nameH * 0.22);
      const sy         = centerY + offset;
      const streakFade = Math.pow(1 - streakT, 2.5) * (0.35 - si * 0.04);

      if (streakFade < 0.005) continue;

      // Width grows then shrinks — cinematic "scan line" feel
      const halfW = easeOut(streakT) * W * (0.45 + si * 0.04) * (1 - streakT * 0.5);

      const grad = ctx.createLinearGradient(centerX - halfW, sy, centerX + halfW, sy);
      grad.addColorStop(0,   `rgba(0, 229, 255, 0)`);
      grad.addColorStop(0.3, `rgba(0, 229, 255, ${streakFade})`);
      grad.addColorStop(0.5, `rgba(200, 245, 255, ${streakFade * 1.4})`);
      grad.addColorStop(0.7, `rgba(0, 229, 255, ${streakFade})`);
      grad.addColorStop(1,   `rgba(0, 229, 255, 0)`);

      ctx.save();
      ctx.beginPath();
      ctx.moveTo(centerX - halfW, sy);
      ctx.lineTo(centerX + halfW, sy);
      ctx.strokeStyle = grad;
      ctx.lineWidth   = 1 + (1 - streakT) * 1.5;
      ctx.stroke();
      ctx.restore();
    }
  }

  // ── 6. CHROMATIC ABERRATION GHOST (pro touch) ──
  // Offset cyan/red "echoes" of the first ring — like lens separation
  if (shockT < 0.40 && shockT > 0.02) {
    const caT      = clamp((shockT - 0.02) / 0.38, 0, 1);
    const eased    = 1 - Math.pow(1 - caT, 4);
    const baseR    = eased * W * 0.68;
    const caAlpha  = Math.pow(1 - caT, 2.5) * 0.22;
    const caOffset = 8 + caT * 14; // px separation

    if (caAlpha > 0.005) {
      // Red channel ghost (shifted left)
      ctx.save();
      ctx.beginPath();
      ctx.arc(centerX - caOffset, centerY, baseR, 0, Math.PI * 2);
      ctx.strokeStyle = `rgba(255, 80, 80, ${caAlpha})`;
      ctx.lineWidth   = 1.2;
      ctx.stroke();
      ctx.restore();
      ...
      ctx.beginPath();
      ctx.arc(centerX + caOffset, centerY, baseR, 0, Math.PI * 2);
      ctx.strokeStyle = `rgba(0, 255, 220, ${caAlpha})`;
      ctx.lineWidth   = 1.2;
      ctx.stroke();
      ctx.restore();
    }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (!window.emailjs || initEmailJS.done) return;
  initEmailJS.done = true;
...
// Fallback — 6 seconds baad force start + observer free karo
...
window.addEventListener('load', () => {
  ...
  // ✅ Width calculate karne ka function — resize pe bhi call hoga
  function recalcWidth() {
    const firstCard = track.children[0];
    const cardW = firstCard ? firstCard.getBoundingClientRect().width + 8 : 130;
    singleSetWidth = totalCards * cardW;
    // offset ko clamp karo taaki jump na ho
    if (offset >= singleSetWidth) offset = offset % singleSetWidth;
  }

  recalcWidth();

  // ✅ Resize pe recalculate karo
