...
function updateMainContent(p) {
  ...
  const slideProgress = clamp((p - 0.70) / 0.30, 0, 1);
  const ease = easeOut(slideProgress);

const vh = window.innerHeight;
const translateY = (1 - ease) * vh * 0.80;
  const opacity    = ease;
...
function canvasDraw() {
  ...
  /* Scroll-driven dark overlay — hero smoothly darkens as content slides up */
  // Starts at p=0.45, fully dark at p=0.95 — no sudden black cut
