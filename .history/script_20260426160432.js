...
function updateHeroText(p) {
  ...
  // fade in (jab hero end ke paas ho)
  if (p > 0.65 && p <= 0.9) {
    const opacity = (p - 0.65) / 0.25;
    heroText.style.opacity = opacity;
  }

  // full visible (center me stable)
  else if (p > 0.9 && p <= 1.05) {
    heroText.style.opacity = 1;
  }

  // fade out (jab next section aaye)
  else if (p > 1.05) {
    const opacity = 1 - (p - 1.05) / 0.2;
...
function canvasDraw() {
  ...
  const p = Math.min(smoothP * 1.3, 1);
  const dpr = window.devicePixelRatio || 1;
...
function canvasTick(now) {
  ...
  smoothP += (rawP - smoothP) * lerpAlpha * 1.1;
