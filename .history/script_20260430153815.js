...
if (!nameEntryDone) {
  if (!nameEntryReady) {
    ...
    if (!nameEntryStart) {
      ...
      nameEntryProgress = 0; // ← fresh start guarantee
    }
    nameEntryProgress = Math.min(
      (performance.now() - nameEntryStart) / NAME_ENTRY_DURATION, 1
    );
...
function canvasTick(now) {
  if (document.hidden) { requestAnimationFrame(canvasTick); return; }
   if (!nameEntryReady && !nameEntryDone)
...
function canvasInit() {
  nameEntryStart = null;      // ← ADD: har init pe fresh start
  nameEntryProgress = 0;      // ← ADD
  canvasResize();
  requestAnimationFrame(canvasTick);
}
      ...
      runProgress(900, () => {
        setTimeout(() => {
          ...
          setTimeout(() => {
           ...
           document.documentElement.scrollTop = 0;
           document.body.scrollTop = 0;
           nameEntryReady = false;       // pehle false karo
           nameEntryProgress = 0;        // progress reset
           nameEntryStart = null;
           nameEntryDone = false;        // ← YEH LINE ADD KARO
           window._revealDone = false;
           requestAnimationFrame(() => { // ek frame wait karo
             nameEntryReady = true;      // tab enable karo
             canvasInit();
});
          }, 700);
        }, 300);
