...
updateVignette(p);
    ...
    // LEFT (soft)
// LEFT (extra soft)
const vL = ctx.createLinearGradient(0, 0, W * 0.18, 0);
...
vR.addColorStop(0, 'rgba(0,0,0,0.10)');
vR.addColorStop(1, 'rgba(0,0,0,0)');
ctx.fillStyle = vR;
ctx.fillRect(0, 0, W, H);

// TOP (barely visible)
const topA = clamp(1 - p * 2, 0, 0.10);
const vT   = ctx.createLinearGradient(0, 0, 0, H * 0.35);
vT.addColorStop(0, `rgba(0,0,0,${topA})`);
vT.addColorStop(1, 'rgba(0,0,0,0)');
ctx.fillStyle = vT;
ctx.fillRect(0, 0, W, H);

// GLOBAL OVERLAY (very controlled)
const darkAlpha = clamp((p - 0.4) / 0.6, 0, 0.18);
