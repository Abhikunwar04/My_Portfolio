...
if (!revealDone && prevRawEntry < BOOM_POINT && rawEntry >= BOOM_POINT) {
  ...
  // ── BOOM EFFECT (YOUR STYLE + FIXED) ──
if (boomStartTime) {
  const elapsed = now - boomStartTime;

  // ⚡ same feel timing (0.72 → 1 ≈ 0.28 range)
  const shockT = clamp(elapsed / 600, 0, 1);
  ...
  // 💥 OPTIONAL SHAKE (feel badhane ke liye)
  if (shockT < 0.3) {
    const shake = (1 - shockT / 0.3) * 8;
    ctx.translate(
