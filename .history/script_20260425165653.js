...
function updateMainContent(p) {
  ...
  // Start sliding in at 55% scroll progress, fully revealed at 100%
  // translateY: 60vh → 0  (matches CSS starting point)
  const slideProgress = clamp((p - 0.55) / 0.45, 0, 1);
  const ease = easeOut(slideProgress);

  const vh = window.innerHeight;
  const translateY = (1 - ease) * vh * 0.60; // 60vh → 0
  const opacity    = clamp((p - 0.55) / 0.25, 0, 1); // fade in 55%→80%
  ...
  if (p >= 0.999) {
    ...
    // Lock mainContent in final position via CSS class
    main.classList.add('settled');
    main.style.transform = '';
    main.style.opacity   = '';

    // Settle hero text into normal DOM flow (it scrolls away naturally now)
    if (heroText) {
      heroText.classList.add('settled');
      heroText.style.opacity = '';
    }

    // Hide the fixed canvas so it doesn't bleed through content
    if (heroCanvas) {
...
function canvasTick() {
  ...
  if (rawP < 0.98 && mainSettled) {
    ...
    if (main)       { main.classList.remove('settled'); main.style.transform = `translateY(${window.innerHeight * 0.6}px)`; main.style.opacity = '0'; }
